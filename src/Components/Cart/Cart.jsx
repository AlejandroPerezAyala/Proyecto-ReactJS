...
const Cart = () => {
  ...
  const {cartProducts, clear, removeItem, getTotalPrice} = useContext(CartContext);

  return(
    <>
      <div className='flex justify-between'>
        <p className='text-3xl text-white mb-3'>TU CARRITO</p>
                                  ...
                                  <>
                                    < div className='bg-white'>
                                      {cartProducts.map((product) => {
                                            return (
                                              <div className=' w-full bg-white border-collapse border-b-2 border-black' key={product.id}> 
                                    ...
                                    <div className='bg-white w-screen flex justify-evenly items-center py-4'>
                                      <h2 className='text-3xl font-medium'>Precio total: ${getTotalPrice()}</h2>
                                      <Link to='/cart/checkout'><button className='bg-black text-white px-3 py-1 text-xl font-medium rounded-lg mt-2 w-60'>Finalizar compra</button></Link>
                                    </div>
                                  </>
                                  }
